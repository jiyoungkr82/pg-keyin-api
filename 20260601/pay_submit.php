...
require __DIR__ . '/KeyinClient.php';
require __DIR__ . '/Database.php';
require __DIR__ . '/PaymentRepository.php';
...
try {
    $insertId = PaymentRepository::insert([
      'tid' => KEYIN_TID,
...
if (isset($status) && $status === 'SUCCESS' && (($result['http_code'] ?? 0) === 200)) {
    ...
    $orderNo = htmlspecialchars($pay_data['order_no'] ?? '');
    $approvalNumber = htmlspecialchars($pay_data['approval_number'] ?? '');
    $goods = htmlspecialchars($goodsName);
    $amountFormatted = number_format((int)($pay_data['amount'] ?? $amount));
    $receiptUrl = htmlspecialchars($pay_data['receipt_url'] ?? '');
    ...
    <body>
        <div style='text-align:center; margin-top:50px;'>
            ...
            <p><strong>주문 번호:</strong> {$orderNo}</p>
            <p><strong>승인 번호:</strong> {$approvalNumber}</p>
            <p><strong>주문 상품:</strong> {$goods}</p>
            <p><strong>결제 금액:</strong> {$amountFormatted}원</p>
            <p><a href='{$receiptUrl}' target='_blank' style='color:blue; text-decoration:underline;'>[영수증(매출전표) 확인하기]</a></p>
            <br>
